...
- **Vercel.** Repo→project links are keyed `(organization_id, repo_owner, repo_name)` — there is no personal variant, so `getVercelProjectLinkByRepo(owner, name)` takes no caller id and two members cannot hold different answers. Vercel API calls still use the acting member's own OAuth token.

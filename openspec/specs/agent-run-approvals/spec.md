# agent-run-approvals Specification

## Purpose

Make an approval a single-use, expiring, server-side record, so execution is authorized by the server rather than by a client assertion.

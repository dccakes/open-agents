...
- **Environment variables**: `ALLOWED_EMAIL_DOMAINS`, `ADMIN_EMAILS`, `DEFAULT_ORG_NAME`/`DEFAULT_ORG_SLUG`. Per the Phase 1 ground rules these go through the Phase 0 config module — which has not landed (`apps/web/lib/config/` does not exist). See `design.md` for how this change sequences against WS-0.1.

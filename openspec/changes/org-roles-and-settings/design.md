...
Better Auth is already the auth layer (`better-auth@^1.6.5`), configured with custom `modelName`/`fields` remapping (`users`, `auth_sessions`, `accounts`, `verification`) and `advanced.database.generateId` using nanoid. Two first-party plugins cover most of what WS-1.0 describes:
...
- **Ground-rule tension, acknowledged:** the Phase 1 ground rules say integration tokens live in env, never DB rows, because preview deployments get a Neon fork of production. The webhook *secret* is not an access token — it grants no API access, only signature verification — but it is still forked into previews. It stays encrypted at rest with the existing Linear AES-GCM helper, and the mitigation for previews is that webhook-triggered runs are gated on `VERCEL_ENV === "production"` in WS-1.3. This exception is deliberate and recorded here so it is not read as an oversight.

### 10. Soft delete + audit are custom; Better Auth has no primitive for them
...
1. **Phase 0 config module sequencing.** The Phase 1 ground rules require env vars to go through the Phase 0 config module and to update `.env.example` and a schema-parse test in the same PR. `apps/web/lib/config/` does not exist yet — WS-0.1 has not landed (only WS-0.2a, the Bun bump, is in `main`). This change introduces four env vars. Either WS-0.1 lands first, or task group 1 introduces a minimal `apps/web/lib/config/auth.ts` in the WS-0.1 shape that WS-0.1 then absorbs. The tasks assume the latter; confirm before implementation.
